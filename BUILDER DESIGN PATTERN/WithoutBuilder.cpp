...
class HttpRequest {
 ...
 public:
 //Construtor with only required parameter (1-arg)
 HttpRequest(const string& url){
    this->url=url;
    this->method="GET";  //default method
    this->timeout=30;    //Default timout
 }
...
int main() {
    ...
    // The problem: what if we forgot to set an important field? :  Ans : Will have issue at runtime not at compile time.
    request4->execute();
};
